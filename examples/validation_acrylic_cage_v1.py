...
ras = [geom.rock.ra, 0.4 * geom.rock.ra, 0.3 * geom.rock.ra, 0.2 * geom.rock.ra,
       0.1 * geom.rock.ra]  # standard vs choked
    ...
    for a in range(0, len(AznDip)):
        ...
        for f in range(0, len(flows)):
            ...
            for r in range(0, len(ras)):
                ...
                #               visuals=False,fname=build_path('stim_w%i_s%i' %(w,a)))

                # circulate
                geom.dyn_stim(Vinj=geom.rock.Vinj, Qinj=geom.rock.Qinj, target=[],
                              visuals=False, fname=build_path('circ_%04d' % (pin)))

                # heat flow
                geom.get_heat(plot=True)
                plt.savefig(build_path('plot_%04d.png' % (pin)), format='png')
                plt.close()

                # show flow model
                if bit + wit == 2:
                    geom.build_vtk(fname=build_path('fini_%04d' % (pin)), vtype=[0, 0, 0, 0, 0, 1])
                if wit == 1:
                    geom.build_vtk(fname=build_path('fini_%04d' % (pin)), vtype=[1, 0, 0, 0, 0, 0])
                geom.build_vtk(fname=build_path('fini_%04d' % (pin)), vtype=[0, 0, 1, 1, 1, 0])
                ...
                geom.save(build_path('inputs_results_valid.txt'), pin, aux=aux, printwells=0)
